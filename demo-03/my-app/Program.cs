...
app.MapGet("/world", async context =>
{
    ...
    var enableExperimentFeature = await client.GetBooleanValue("experiment", true, EvaluationContext.Builder().Set("email", email).Build());
    ...
    if (enableExperimentFeature)
    {
        ...
        await context.Response.WriteAsJsonAsync(new { message = "Hello the whole new world!" });
    }
    else
    {
